...
;;
  ...
  daily_infections_fooddistro
  daily_infections_waterpoint
  daily_infections_shelter
  daily_infections_latrine
  daily_infections_hcfac
...
;;
  ...
  set number_dailyinfections_perlocation ["fooddistro" 0 "waterpoint" 0 "latrine" 0 "shelter" 0 "hc-fac" 0]
...
;; post GL stuff
  ...
  ifelse Hour = 22 and Minute = 22 [
    ...
    set number_dailyinfections_perlocation replace-item 1 number_dailyinfections_perlocation daily_infections_fooddistro
    set number_dailyinfections_perlocation replace-item 3 number_dailyinfections_perlocation daily_infections_waterpoint
    set number_dailyinfections_perlocation replace-item 5 number_dailyinfections_perlocation daily_infections_latrine
    set number_dailyinfections_perlocation replace-item 7 number_dailyinfections_perlocation daily_infections_shelter
    set number_dailyinfections_perlocation replace-item 9 number_dailyinfections_perlocation daily_infections_hcfac

    ;output-print number_dailyinfections_perlocation

    set daily_infections_fooddistro 0
    set daily_infections_waterpoint 0
    set daily_infections_shelter 0
    set daily_infections_latrine 0
    set daily_infections_hcfac 0 ]
  [set attitude-and-infections [] set number_dailyinfections_perlocation ["fooddistro" 0 "waterpoint" 0 "latrine" 0 "shelter" 0 "hc-fac" 0]]
...
to initiate-corona
  ...
  set comb_maxmin_queuingtime_cooperative [0 0]
  set comb_maxmin_queuingtime_competitive [0 0]
  set comb_maxmin_queuingtime_newcompetitive [0 0]
end
...
to become-infected
  ...
  if any? tents with [(infection = "pre-symptomatic" or infection = "symptomatic" or infection = "asymptomatic") and (attitude = "competitive" or attitude = "new-competitive")] [set nbr-infectious-competitive count infectious-competitive]
  ...
  if any? tents with [(infection = "pre-symptomatic" or infection = "symptomatic" or infection = "asymptomatic") and attitude = "cooperative"] [set nbr-infectious-cooperative count infectious-cooperative]
  ...
  if any? tents with [(infection = "pre-symptomatic" or infection = "symptomatic" or infection = "asymptomatic")] [set nbr-infectious count total-infectious]
  ...
  ifelse where-did-i-get-infected = one-of fooddistros [set daily_infections_fooddistro daily_infections_fooddistro + 1]
    [ifelse where-did-i-get-infected = one-of waterpoints [set daily_infections_waterpoint daily_infections_waterpoint + 1]
      [ifelse where-did-i-get-infected = one-of latrines [set daily_infections_latrine daily_infections_latrine + 1]
        [ifelse where-did-i-get-infected = one-of hc-facs [set daily_infections_hcfac daily_infections_hcfac + 1]
          [set daily_infections_shelter daily_infections_shelter + 1] ]]]
  ;if where-did-i-get-infected = one-of fooddistros [set daily_infections_fooddistro daily_infections_fooddistro + 1]
  ;if where-did-i-get-infected = one-of waterpoints [set daily_infections_waterpoint daily_infections_waterpoint + 1]
  ;if where-did-i-get-infected = one-of latrines [set daily_infections_latrine daily_infections_latrine + 1]
  ;if where-did-i-get-infected = one-of hc-facs [set daily_infections_hcfac daily_infections_hcfac + 1]
  ;if (where-did-i-get-infected = one-of patches) or (where-did-i-get-infected = "none") [set daily_infections_shelter daily_infections_shelter + 1]
...
to setup-foodpoint
  ...
  [ setxy 88 55
    ...
    if policy-implemented = "policy 0 (baseline)" [set service-time service-timeSA]     ; baseline: 20% of the population attends it (head of households)
...
to refugee-served
  ...
  if [serving-waiting-list] of current-task != [] [                                                          ; to guarantee we only do this when the list isn't empty
    ...
    if attitude = "cooperative" and [physical-waiting-list] of current-task != [] [                          ; make sure that physical list is not empty otherwise it will give an error
      ...
      set comb_maxmin_queuingtime_cooperative replace-item 1 comb_maxmin_queuingtime_cooperative min_queuing_time_cooperative
    ...
    if attitude = "competitive" [
      ...
      set comb_maxmin_queuingtime_competitive replace-item 1 comb_maxmin_queuingtime_competitive min_queuing_time_competitive
      update-serving-queue                                                                                  ; update the serving queue
      set current-task -50                              ; set the current-task of the refugee who was served to -50 so it goes back into wandering around again
      set number-in-physical-queue -50                  ; set attribute to after queuing value (helps debugging)
      set number-in-serving-queue -50                   ; ""
    ...
    if attitude = "new-competitive" [
      ...
      set comb_maxmin_queuingtime_newcompetitive replace-item 1 comb_maxmin_queuingtime_newcompetitive min_queuing_time_newcompetitive
      update-serving-queue                                                                                         ; update the serving queue
...
@#$#@#$#@
<experiments>
  <experiment name="SA_All" repetitions="10" runMetricsEveryStep="true">
    <setup>setup
initiate-corona
create-COVID-facility</setup>
  ...
  <experiment name="Ex99" repetitions="3" runMetricsEveryStep="true">
...
export-output (word "Ex99_Policy_" policy-implemented "_scenario_" percentage-competitive "_run_" behaviorspace-run-number)</final>
    ...
    <metric>Minute</metric>
    <metric>count tents</metric>
    <metric>count tents with [infected? = true]</metric>
    <metric>cum-dead</metric>
    <metric>cum-recovered</metric>
    <metric>trackingTimeInQueue_average_output</metric>
    <metric>trackingTimeInQueue_cooperative_output</metric>
  ...
  <experiment name="ExSA" repetitions="10" runMetricsEveryStep="true">
    <setup>default-values
setup
initiate-corona
create-COVID-facility</setup>
    <go>go</go>
    <timeLimit steps="43200"/>
    <metric>ticks</metric>
    <metric>Day</metric>
    <metric>Hour</metric>
    <metric>Minute</metric>
